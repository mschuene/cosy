...
**** Focus view-apply visual routines and count the objects of interest
     #+begin_src clojure :ns cosy.focus :tangle src/cosy/focus.clj
       ...
       (defn check-constraints [position colors shapes]
         (prn "positioniii " position (mapv + (:offset @mem/memory) position))
         (let [[colc shapec] (get (:obscured @mem/memory)
                                  (mapv + (:offset @mem/memory) position))]
           (prn "colc shapec" colc shapec)
           (if (and colc shapec)
       ...
       (defn adjacent-cells [region color-pred? shape-pred? [x y]]
         (let [directions [[0 0] [-1 0] [0 -1] [1 0] [0 1]]]
           (->> directions
                ...
                (#(do (prn %) %))
                (remove empty?)
                (filter #(matches? region % color-pred? shape-pred?))
                (#(do (prn "after filter " %) %))
       ...
       ;;todo add offset in :clusters attribute of recognized objects
       ;;to be able to count obscured objects 
     #+end_src 

     #+RESULTS:
     : #'cosy.focus/focus-view-shapes
