...
public class Main {
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        while(on) {
            ...
            Group[] group = findGroup(line.split(" "));
     ...
     *
     */
    public static Group[] findGroup(String[] args) {
        ArrayList<Group> group = new ArrayList<Group>(2);
        for(String next : args) {
            try {
            	if(next.charAt(next.length()-1) == 's') {
            		group.add(Group.valueOf(next.substring(0, next.length()-1).toUpperCase()));
            	} else {
            		group.add(Group.valueOf(next.toUpperCase()));
            	}

            } catch(IllegalArgumentException e) {
            }
        }
        return group.toArray(new Group[group.size()]);
    }
}
